...
if ( false !== czr_get( 'query' ) ) {
  $_query = new WP_Query( czr_get( 'query' ) );
  $wp_query = $_query;
}
do_action( '__before_tc_loop', czr_get( 'id' ) );
if ( have_posts() && ! is_404() ) {
  while ( have_posts() ) {
    ...
    if ( 'main_loop' == czr_get('id') ) {
      ...
      elseif ( czr_has('singular_article') ) {
        czr_render_template('content/singles/article','singular_article');
      }
    }
    else {
      //if this is a custom loop, use a dynamic action hook to load a custom query loop
      do_action( 'in_' . czr_get('id') );
    }

  }//endwhile;
}
do_action( '__after_tc_loop', czr_get( 'id' ) );
