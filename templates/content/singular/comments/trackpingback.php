...
<li <?php comment_class() ?> id="comment-<?php comment_ID() ?>" <?php czr_fn_echo('element_attributes') ?>>
  <a href="<?php echo esc_url( get_comment_author_url() ) ?>"><sup><?php czr_fn_echo('ping_number') ?></sup><span class="font-weight-bold excerpt"><?php comment_excerpt() ?></span> - <?php comment_author() ?></a>
  <?php
  if ( czr_fn_has('edit_button') && (bool) $edit_comment_link = get_edit_comment_link() )
    czr_fn_render_template(
      'modules/common/edit_button', 'edit_button',
