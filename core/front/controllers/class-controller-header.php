  ...
  class CZR_cl_controller_header extends CZR_cl_controllers {
    ...
    function tc_display_view_favicon() {
      ...
      if ( function_exists('has_site_icon') && has_site_icon() )
        return;
      $_fav_option  			= esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_fav_upload') );
     	...
     	if ( is_numeric($_fav_option) ) {
     		...
     		$_saved_path 		= esc_url ( CZR_cl_utils::$inst->tc_opt( 'tc_fav_upload') );
    ...
    function tc_display_view_tagline() {
      if ( ! isset( $this -> _cache[ 'view_tagline' ] ) )
        $this -> _cache[ 'view_tagline' ] = CZR___::$instance -> tc_is_customizing() || ! ( 0 == esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_show_tagline') ) );
    ...
    function tc_display_view_logo() {
      ...
      $logo_option          = CZR_cl_utils::$inst->tc_opt( "tc_logo_upload");
      if ( $logo_option ) {
    ...
    function tc_display_view_sticky_logo() {
      ...
      $sticky_logo_option = CZR_cl_utils::$inst->tc_opt( "tc_sticky_logo_upload");

      if ( ! esc_attr( $sticky_logo_option ) )
        $to_return = false;
      elseif ( ! ( esc_attr( CZR_cl_utils::$inst->tc_opt( "tc_sticky_header") ) &&
            esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_sticky_show_title_logo') ) ) )
        $to_return = false;
      else {
        $to_return = false;
    ...
    function tc_display_view_sidenav() {
      if ( ! isset( $this -> _cache[ 'view_sidenav' ] ) )
        $this -> _cache[ 'view_sidenav' ] = $this -> tc_display_view_menu() && 'aside' == esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_menu_style' ) );
      return $this -> _cache[ 'view_sidenav' ];
    }

    function tc_display_view_menu() {
      if ( ! isset( $this -> _cache[ 'view_menu' ] ) )
        $this -> _cache[ 'view_menu' ] =  ! ( (bool) CZR_cl_utils::$inst->tc_opt('tc_hide_all_menus') );
