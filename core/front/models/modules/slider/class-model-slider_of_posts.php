...
class CZR_cl_slider_of_posts_model_class extends CZR_cl_slider_model_class {
  ...
  */
  protected function czr_get_the_slides( $slider_name_id, $img_size ) {
    ...
    */
    ...
    return apply_filters( 'tc_the_slides', $this -> czr_get_the_posts_slides( $slider_name_id, $img_size, $load_transient , $store_transient ) );
  }
  ...
  */
  private function czr_get_pre_posts_slides( $args ){
    $defaults       = array(
      ...
      //options
      'stickies_only'       => esc_attr( CZR_cl_utils::$inst->czr_opt( 'tc_posts_slider_stickies' ) ),
      'show_title'          => esc_attr( CZR_cl_utils::$inst->czr_opt( 'tc_posts_slider_title' ) ),
      'show_excerpt'        => esc_attr( CZR_cl_utils::$inst->czr_opt( 'tc_posts_slider_text' ) ),
      'button_text'         => esc_attr( CZR_cl_utils::$inst->czr_opt( 'tc_posts_slider_button_text' ) ),
      'limit'               => esc_attr( CZR_cl_utils::$inst->czr_opt( 'tc_posts_slider_number' ) ),
      'link_type'           => esc_attr( CZR_cl_utils::$inst->czr_opt( 'tc_posts_slider_link') ),
    );
    ...
    $smart_load_enabled = 1 == esc_attr( CZR_cl_utils::$inst->czr_opt( 'tc_img_smart_load' ) );
    ...
    if ( version_compare( $GLOBALS['wp_version'], '4.4', '>=' ) )
      $args['slider_responsive_images'] = 0 == esc_attr( CZR_cl_utils::$inst->czr_opt('tc_resp_slider_img') ) ? false : true ;
    /* Get the pre_model */
  ...
  */
  function czr_get_single_post_slide_pre_model( $_post , $img_size, $args ){
    $ID                     = $_post->ID;
    //attachment image
    $thumb                  = CZR_cl_utils_thumbnails::$instance -> czr_get_thumbnail_model($img_size, $ID, null, isset($args['slider_responsive_images']) ? $args['slider_responsive_images'] : null );
  ...
  */
  function tc_cache_posts_slider( $args = array() ) {
    $defaults = array (
      ...
      'img_size'        => 1 == CZR_cl_utils::$inst->czr_opt( 'tc_slider_width' ) ? 'slider-full' : 'slider',
