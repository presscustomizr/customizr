...
class CZR_cl_header_model_class extends CZR_cl_Model {
  ...
  */
  function tc_extend_params( $model = array() ) {
    $element_class = apply_filters('tc_header_classes', array(
        ...
        'logo-' . esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_header_layout' ) )
    ));
  ...
  function tc_has_pusher_margin_top() {
    ...
    return esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_sticky_header' ) )
  ...
  */
  function tc_user_options_style_cb( $_css ) {
    //TOP BORDER
    if ( 1 != esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_top_border') ) ) {
      $_css = sprintf("%s%s",
        $_css,
        "
    ...
    //HEADER Z-INDEX
    if ( 100 != esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_sticky_z_index') ) ) {
      $_custom_z_index = esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_sticky_z_index') );
      $_css = sprintf("%s%s",
        $_css,
        "
  ...
  function tc_body_class( $_classes/*array*/ ) {
    //STICKY HEADER
    if ( 1 == esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_sticky_header' ) ) ) {
      array_push( $_classes, 'tc-sticky-header', 'sticky-disabled' );

      //STICKY TRANSPARENT ON SCROLL
      if ( 1 == esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_sticky_transparent_on_scroll' ) ) )
