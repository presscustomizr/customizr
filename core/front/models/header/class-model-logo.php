  ...
  */
  function tc_extend_params( $model = array() ) {
    ...
    $model[ 'element_attributes' ] = trim( sprintf('%1$s %2$s %3$s %4$s',
        ...
        implode(' ' , apply_filters('tc_logo_other_attributes' , ( 0 == CZR_cl_utils::$inst->tc_opt( 'tc_retina_support' ) ) ? array('data-no-retina') : array() ) )
    ));
  ...
  function tc_get_logo_src_args( $logo_type ) {
    ...
    $_logo_option  			= esc_attr( CZR_cl_utils::$inst->tc_opt( "tc{$logo_type_sep}logo_upload") );
    ...
    if ( is_numeric($_logo_option) ) {
      ...
      $_saved_path 			= esc_url ( CZR_cl_utils::$inst->tc_opt( "tc{$logo_type_sep}logo_upload") );
    ...
    $logo_resize 			= ( $logo_type_sep == '_' ) ? esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_logo_resize') ) : '';
  ...
  */
  function tc_user_options_style_cb( $_css ) {
    ...
    if ( CZR___::$instance -> tc_is_customizing() ||
        ( 0 != esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_sticky_header') ) && 0 != esc_attr( CZR_cl_utils::$inst->tc_opt( 'tc_sticky_shrink_title_logo') ) ) ) {
