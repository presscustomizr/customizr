...
class CZR_cl_tagline_model_class extends CZR_cl_Model {
  ...
  */
  function czr_fn_extend_params( $model = array() ) {
    $model[ 'element_attributes' ] = ( CZR() -> czr_fn_is_customizing() && 0 == esc_attr( CZR_cl_utils::$inst->czr_fn_opt( 'tc_show_tagline') ) ) ? 'style="display:none;"' : '';
  ...
  */
  function pre_rendering_view_header_cb( $header_model ) {
    ...
    if ( esc_attr( CZR_cl_utils::$inst->czr_fn_opt( "tc_sticky_header") || CZR() -> czr_fn_is_customizing() ) ) {
      $_class =        0 != esc_attr( CZR_cl_utils::$inst->czr_fn_opt( 'tc_sticky_show_tagline') ) ? 'tc-tagline-on' : 'tc-tagline-off';
